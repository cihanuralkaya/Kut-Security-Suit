...
// Kullanım:
//
//	go run ./tools/gencerts -out ./dev-certs -name kut-c2
package main
...
func main() {
	...
	name := flag.String("name", "kut-c2", "sunucu adı (TLS SAN / doğrulama adı)")
...
func run(outDir, serverName string, days int) error {
	...
	caTmpl := &x509.Certificate{
		...
		Subject:               pkix.Name{CommonName: "KUT Dev CA", Organization: []string{"KUT"}},
	...
	// 32 baytlık rastgele ana anahtar (base64) — KUT_MASTER_KEY için öneri.
	master := make([]byte, 32)
	...
	fmt.Printf("  export KUT_MASTER_KEY=%s\n", base64.StdEncoding.EncodeToString(master))
	fmt.Printf("  export KUT_CA_CERT=%s\n", filepath.Join(outDir, "ca.crt"))
	fmt.Printf("  export KUT_CA_KEY=%s\n", filepath.Join(outDir, "ca.key"))
	fmt.Printf("  export KUT_SERVER_CERT=%s\n", filepath.Join(outDir, "server.crt"))
	fmt.Printf("  export KUT_SERVER_KEY=%s\n", filepath.Join(outDir, "server.key"))
	fmt.Printf("  # ajan tarafı: KUT_CA_PEM=%s  KUT_SERVER_NAME=%s\n",
