// Command mkicon, KUT logosundan (kaynak PNG) uygulama ikonlarını üretir:
//   - assets/kut.ico            (çok-boyutlu, PNG-gömülü Windows ikonu)
//   - assets/kut-icon-256.png   (kare önizleme / sosyal görsel)
...
// Kullanım:
//
//	go run ./tools/mkicon [-src assets/kut-logo.png] [-cx 825 -cy 390 -side 720]
package main
...
func main() {
	src := flag.String("src", "assets/kut-logo.png", "kaynak logo (PNG/JPEG)")
...
func run(src, root string, cx, cy, side int) error {
	...
	// assets/kut.ico
	ico := writeICO(imgs)
	if err := os.WriteFile(filepath.Join(root, "assets", "kut.ico"), ico, 0o644); err != nil {
		return err
	}
	fmt.Printf("yazıldı  assets/kut.ico            (%d boyut, %d bayt)\n", len(imgs), len(ico))

	// assets/kut-icon-256.png (kare önizleme)
	preview := resizeArea(square, 256)
	pf, err := os.Create(filepath.Join(root, "assets", "kut-icon-256.png"))
	if err != nil {
		return err
	}
	...
	pf.Close()
	fmt.Println("yazıldı  assets/kut-icon-256.png")
