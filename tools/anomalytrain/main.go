...
// formatına yazar. Böylece "eğit → JSON → ajan (KUT_ANOMALY_MODEL)" hattı
// tamamlanır.
//
...
func main() {
	...
	if *signKey != "" {
		...
		fmt.Printf("imza yazıldı: %s.sig (ajanda KUT_ANOMALY_PUBKEY ile doğrulanır)\n", *out)
	}
}
