...
// Relu/Sigmoid) KUT anomali motorunun taşınabilir JSON ağırlık formatına çevirir.
...
func main() {
	...
	if *out == "" {
		...
		if *signKey != "" {
			...
			fmt.Fprintf(os.Stderr, "onnx2json: imza yazıldı: %s.sig (ajanda KUT_ANOMALY_PUBKEY ile doğrulanır)\n", *out)
		}
	}
