...
KUT_CA_CERT=$PKI_DIR/ca.crt
KUT_CA_KEY=$PKI_DIR/ca.key
KUT_SERVER_CERT=$PKI_DIR/server.crt
KUT_SERVER_KEY=$PKI_DIR/server.key
KUT_LISTEN_AGENT=:8443
KUT_LISTEN_ENROLL=:8444
KUT_LISTEN_ADMIN=:8445
KUT_DEMO_ADMIN_EMAIL=admin@local
KUT_DEMO_ADMIN_PASSWORD=$ADMIN_PASS
KUT_RETENTION_DAYS=90
EOF
  chmod 600 "$ENV_FILE"
  echo "Yapılandırma üretildi: $ENV_FILE"
  echo ">>> Konsol girişi: admin@local / $ADMIN_PASS  (DEMO/bellek-içi mod)"
  echo ">>> Üretim için: KUT_DATABASE_URL ayarlayın ve tools/adminseed ile yönetici ekleyin."
else
  echo "Mevcut yapılandırma korunuyor: $ENV_FILE"
fi

cat > /etc/systemd/system/kut-c2.service <<UNITEOF
[Unit]
Description=KUT C2 (Command & Control)
...
systemctl enable --now kut-c2.service
echo "KUT C2 kuruldu ve başlatıldı (systemctl status kut-c2)."
