...
func Enroll(ctx context.Context, addr string, caPEM []byte, serverName, token string, csrPEM []byte, info EnrollInfo) (*kutv1.EnrollResponse, error) {
	...
	cli := kutv1.NewEnrollmentServiceClient(conn)
	return cli.Enroll(ctx, &kutv1.EnrollRequest{
...
func DialAgent(addr string, holder *CertHolder, caPEM []byte, serverName string) (kutv1.AgentServiceClient, *grpc.ClientConn, error) {
	...
	return kutv1.NewAgentServiceClient(conn), conn, nil
}
...
func Renew(ctx context.Context, enrollAddr string, holder *CertHolder, caPEM []byte, serverName string) (newCertPEM, newKeyPEM []byte, notAfter time.Time, err error) {
	...
	resp, err := kutv1.NewEnrollmentServiceClient(conn).RenewCertificate(ctx, &kutv1.RenewRequest{CsrPem: csrPEM})
	if err != nil {
