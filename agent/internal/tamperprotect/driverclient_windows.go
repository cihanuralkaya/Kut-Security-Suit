...
// Windows filter-manager (fltlib.dll) istemcisi: kutflt.sys ile \KutFltPort
// üzerinden konuşur — canlı durum (GET_STATUS) okur ve kurcalama olaylarını akıtır.
// ABI, driver/kutflt/inc/kutflt_ioctl.h ile BAYT-UYUMLU olmalıdır.
...
const driverPortName = `\KutFltPort`

// kutRequest ~ KUTFLT_REQUEST
type kutRequest struct {
	Command uint32
	Arg     uint32
}

// kutStatus ~ KUTFLT_STATUS
type kutStatus struct {
	Version        uint32
	Active         uint32
...
// kutEvent ~ KUTFLT_EVENT (packed; doğal hizalama zaten dolgusuz).
type kutEvent struct {
	Kind         uint32
	ActorPid     uint32
	TimestampQpc uint64
	Target       [260]uint16
...
// DriverStatus, \KutFltPort'a bağlanır ve GET_STATUS ister. Başarıda
...
func DriverStatus() (active bool, deniedOps uint64, err error) {
	...
	in := kutRequest{Command: 1} // KutFltCmdGetStatus
	var out kutStatus
	var retLen uint32
