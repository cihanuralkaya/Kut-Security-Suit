...
func TestAssessKernel(t *testing.T) {
	p := Assess(Defenses{Watchdog: true}, true, "kutflt")
	if p.Level != "kernel" || !p.KernelDriver || p.KernelDriverName != "kutflt" {
...
func TestKernelDriverProbeDefaultAbsent(t *testing.T) {
	...
	if present {
		t.Skip("beklenmedik: ortamda bir kutflt.sys mevcut — atlanıyor")
	}
}
