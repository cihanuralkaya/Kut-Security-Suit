...
// LUKS aygıtı AÇIKÇA KUT_WIPE_DEVICE ile verilmelidir (otomatik tahmin YOK). Yalnız
// ARM'lı (KUT_ALLOW_WIPE=1) ve güvenli-mod KAPALI iken çağrılır (agent denetler).
func Wipe() error {
	dev := os.Getenv("KUT_WIPE_DEVICE")
	if dev == "" {
		return errors.New("deviceaction: KUT_WIPE_DEVICE ayarlı değil (silinecek LUKS aygıtı; yanlış-aygıt koruması)")
	}
