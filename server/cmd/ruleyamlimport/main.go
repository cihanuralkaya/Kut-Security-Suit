// Command ruleyamlimport, YAML tespit kurallarını (YAML) KUT yerel tespit kural
// biçimine (JSON) çevirir. Çıktı, KUT_DETECT_RULES_FILE ile C2'ye verilebilir.
//
//	go run ./server/cmd/ruleyamlimport -in rules.yml > detect-rules.json
//	go run ./server/cmd/ruleyamlimport -in ./ruleyaml-dir/ -out detect-rules.json
//
// Çok-belgeli YAML (--- ayraçlı) ve dizin (özyinelemesiz *.yml/*.yaml) desteklenir.
// Desteklenmeyen YAML yapıları (OR/NOT/1-of/liste değerleri) GÜVENLE atlanır ve
...
import (
	...
	"kut.corp/suite/server/internal/ruleyaml"
)

func main() {
	in := flag.String("in", "", "YAML YAML dosyası veya dizini")
	...
	if len(files) == 0 {
		log.Fatal("YAML YAML dosyası bulunamadı")
	}
	...
	for _, f := range files {
		...
		rs, skips, err := ruleyaml.ConvertMulti(data)
		if err != nil {
