...
type Set struct {
	...
	// Mesaj-alt-dize taraması için ön-derlenmiş çok-desenli eşleştirici (Aho-Corasick):
	// gösterge sayısından bağımsız TEK geçiş. vals, desen indeksi → gösterge değeri
	// eşlemesidir (deterministik sıra). Load sonrası kurulur; kurulmamışsa (nil)
	// doğrusal taramaya geri düşülür.
	vals    []string
	matcher *mpm.Matcher
}
...
// buildMatcher, mesaj-alt-dize taraması için gösterge değerleri üzerinde bir
// Aho-Corasick eşleştirici kurar. Değerler deterministik (sıralı) tutulur; desen
// indeksi doğrudan vals indeksidir. Böylece MatchIndicator adım-2, göstergeleri
// tek tek dolaşmak (O(gösterge×mesaj)) yerine mesajı TEK kez tarar (O(mesaj)).
func (s *Set) buildMatcher() {
	s.vals = make([]string, 0, len(s.byValue))
	for v := range s.byValue {
		s.vals = append(s.vals, v)
	}
	sort.Strings(s.vals)
	s.matcher = mpm.Build(s.vals)
}
...
func (s *Set) MatchIndicator(details map[string]any, message string) (ind Indicator, indicator string, ok bool) {
	...
	// Aho-Corasick ile TEK geçişte taranır (gösterge sayısından bağımsız). Eşleşme
	// varsa metinde EN ERKEN biten göstergeyi döndürür (deterministik — eski map
	// iterasyonu sıra garanti etmiyordu; davranış: "bir eşleşme varsa döndür" korunur).
	if s.matcher != nil {
		if idxs := s.matcher.Matches(message); len(idxs) > 0 {
			val := s.vals[idxs[0]]
			return s.byValue[val], val, true
		}
		return Indicator{}, "", false
	}
	// Geri düşüş: eşleştirici kurulmamışsa (ör. Set doğrudan kurulmuşsa) doğrusal tarama.
