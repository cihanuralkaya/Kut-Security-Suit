// Package ruleyaml, topluluk YAML tespit kurallarını (YAML) KUT'in yerel tespit
// kural modeline (detect.Rule) çevirir. Böylece kuruluşlar yaygın YAML içeriğini
// koda dokunmadan içe aktarabilir.
//
// KAPSAM (v1, dürüst alt küme): detect motoru alan-alt-dize (AND) + mesaj-alt-dize
// (AND) eşleşmesi yaptığından, YAML'nın YALNIZ bununla ifade edilebilen alt kümesi
// desteklenir:
...
// ruleyamlDoc, desteklenen YAML alanlarıdır.
type ruleyamlDoc struct {
...
// levelToSeverity, YAML level'ını KUT önem düzeyine eşler.
...
var (
	...
	ErrNoDetection = errors.New("ruleyaml: detection/condition yok")
	// ErrUnsupported, kural desteklenmeyen bir yapı içerdiğinde döner.
	ErrUnsupported = errors.New("ruleyaml: desteklenmeyen yapı")
	// ErrEmptyRule, çevrilen kuralın hiç eşleşme koşulu olmadığında döner.
	ErrEmptyRule = errors.New("ruleyaml: boş kural (eşleşme koşulu yok)")
)

// Convert, tek bir YAML YAML belgesini bir detect.Rule'a çevirir.
func Convert(data []byte) (detect.Rule, error) {
	var doc ruleyamlDoc
	if err := yaml.Unmarshal(data, &doc); err != nil {
		return detect.Rule{}, fmt.Errorf("ruleyaml: YAML çözülemedi: %w", err)
	}
	return convertDoc(doc)
}

func convertDoc(doc ruleyamlDoc) (detect.Rule, error) {
	if strings.TrimSpace(doc.Title) == "" {
		return detect.Rule{}, errors.New("ruleyaml: title zorunlu")
	}
...
// ConvertMulti, çok-belgeli bir YAML YAML'ını (--- ayraçlı) çevirir. Başarılı
