...
// FromHash, ÖNCEDEN HESAPLANMIŞ bir SHA-256 özetinden (ör. toplanırken hesaplanmış
// artefakt) delil kaydı üretir — içeriği yeniden okumadan. Genesis COLLECTED gözetim
// kaydıyla zinciri başlatır. now nil ise time.Now.
func FromHash(id, deviceID, collector, method, eventRef, sha256hex string, size int64, now func() time.Time) Evidence {
	if now == nil {
		now = time.Now
	}
	at := now().UTC()
