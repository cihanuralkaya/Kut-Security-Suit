...
func TestFromHash(t *testing.T) {
	clock := fixedClock(testBase, time.Minute)
	e := FromHash("ev-h", "dev-2", "kut-agent", "live-collect", "evt-9", "abc123", 4096, clock)
	if e.SHA256 != "abc123" || e.Size != 4096 {
		t.Fatalf("önceden hesaplanmış hash/size korunmalı: %+v", e)
	}
	if len(e.Custody) != 1 || e.Custody[0].Action != ActionCollected {
		t.Fatalf("genesis COLLECTED gözetim kaydı olmalı: %+v", e.Custody)
	}
	if ok, at := e.Verify(); !ok || at != -1 {
		t.Errorf("zincir sağlam olmalı: ok=%v at=%d", ok, at)
	}
}
