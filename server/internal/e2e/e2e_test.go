...
import (
	...
	"kut.corp/suite/server/internal/ota"
...
func TestEndToEnd(t *testing.T) {
	...
	renewResp, err := renewCli.RenewCertificate(ctx, &kutv1.RenewRequest{CsrPem: renewCSR})
	renewConn.Close()
	if err != nil {
	...
	_, err = kutv1.NewEnrollmentServiceClient(noCertConn).RenewCertificate(ctx, &kutv1.RenewRequest{CsrPem: renewCSR})
	noCertConn.Close()
	if err == nil {
	...
	hb, err := cli.Heartbeat(ctx, &kutv1.HeartbeatRequest{
		Identity:             &kutv1.AgentIdentity{DeviceId: enrollResp.GetDeviceId(), AgentVersion: "test", OsPlatform: "test"},
	...
	if err := stream.Send(&kutv1.EventBatch{
		Identity: &kutv1.AgentIdentity{DeviceId: enrollResp.GetDeviceId()},
		Events: []*kutv1.Event{{
			Sequence:   1,
			Category:   kutv1.EventCategory_EVENT_CATEGORY_SECURITY,
			Severity:   kutv1.Severity_SEVERITY_HIGH,
	...
	store.setPolicy(&kutv1.PolicyBundle{
		PolicyVersion: "v1",
		Rules: []*kutv1.PolicyRule{{
			RuleId:      "r1",
			Type:        kutv1.PolicyRule_RULE_TYPE_APP_TIME_BLOCK,
	...
	defer psCancel()
	ps, err := cli.StreamPolicies(psCtx, &kutv1.PolicySubscribeRequest{
		Identity: &kutv1.AgentIdentity{DeviceId: enrollResp.GetDeviceId()}, CurrentPolicyVersion: "",
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	store.setPolicy(&kutv1.PolicyBundle{PolicyVersion: "v2"})
	...
	payload := []byte("KUT agent 1.4.0 ikilisi")
	...
	store.setUpdate(&kutv1.UpdateManifest{
	...
	upd, err := cli.CheckUpdate(ctx, &kutv1.UpdateCheckRequest{
		Identity: &kutv1.AgentIdentity{DeviceId: enrollResp.GetDeviceId(), AgentVersion: "0.1.0-dev", OsPlatform: "test"},
	})
	...
	gated, err := cli.CheckUpdate(ctx, &kutv1.UpdateCheckRequest{
		Identity: &kutv1.AgentIdentity{DeviceId: enrollResp.GetDeviceId(), OsPlatform: "test"},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	store.enqueueCmd(&kutv1.Command{CommandId: "c1", Type: kutv1.Command_COMMAND_TYPE_QUARANTINE})
	hb2, err := cli.Heartbeat(ctx, &kutv1.HeartbeatRequest{
		Identity: &kutv1.AgentIdentity{DeviceId: enrollResp.GetDeviceId()},
	})
	if err != nil {
		t.Fatal(err)
	}
	if len(hb2.GetPendingCommands()) != 1 || hb2.GetPendingCommands()[0].GetType() != kutv1.Command_COMMAND_TYPE_QUARANTINE {
	...
	hb3, err := cli.Heartbeat(ctx, &kutv1.HeartbeatRequest{Identity: &kutv1.AgentIdentity{DeviceId: enrollResp.GetDeviceId()}})
	if err != nil {
		t.Fatal(err)
	}
	...
	defer rcancel()
	if _, err := revokedCli.Heartbeat(rctx, &kutv1.HeartbeatRequest{
		Identity: &kutv1.AgentIdentity{DeviceId: enrollResp.GetDeviceId()},
	}); err == nil {
...
type memStore struct {
	...
	policy   *kutv1.PolicyBundle
	update   *kutv1.UpdateManifest
	pendCmds []*kutv1.Command
	seq      int
}
...
func (m *memStore) TenantForDevice(_ context.Context, _ string) (string, error) {
	return "", nil
}
...
func (m *memStore) enqueueCmd(c *kutv1.Command) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.pendCmds = append(m.pendCmds, c)
}

func (m *memStore) PendingCommands(_ context.Context, _ string) ([]*kutv1.Command, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	out := m.pendCmds
	m.pendCmds = nil // teslim edildi
	return out, nil
}
...
func (m *memStore) CurrentPolicy(_ context.Context, _ string) (*kutv1.PolicyBundle, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	return m.policy, nil
}

func (m *memStore) setUpdate(u *kutv1.UpdateManifest) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.update = u
}

func (m *memStore) LatestUpdate(_ context.Context, _, _, _ string) (*kutv1.UpdateManifest, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	return m.update, nil
}
...
func doEnroll(t *testing.T, ctx context.Context, addr string, caPEM []byte, token string, csrPEM []byte) *kutv1.EnrollResponse {
	t.Helper()
...
func doEnrollErr(ctx context.Context, addr string, caPEM []byte, token string, csrPEM []byte) (*kutv1.EnrollResponse, error) {
	...
	defer conn.Close()
	return kutv1.NewEnrollmentServiceClient(conn).Enroll(ctx, &kutv1.EnrollRequest{
		EnrollmentToken: token, CsrPem: csrPEM, Hostname: "test-host", MacAddress: "aa:bb:cc:dd:ee:ff", OsInfo: "test",
	})
}

func dialAgent(t *testing.T, addr string, clientCertPEM, clientKeyPEM, caPEM []byte) (kutv1.AgentServiceClient, *grpc.ClientConn) {
	t.Helper()
	...
	return kutv1.NewAgentServiceClient(conn), conn
}

func dialEnrollMTLS(t *testing.T, addr string, clientCertPEM, clientKeyPEM, caPEM []byte) (kutv1.EnrollmentServiceClient, *grpc.ClientConn) {
	t.Helper()
	cert, err := tls.X509KeyPair(clientCertPEM, clientKeyPEM)
	if err != nil {
		t.Fatal(err)
	...
	return kutv1.NewEnrollmentServiceClient(conn), conn
}
