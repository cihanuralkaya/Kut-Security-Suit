...
func sampleExtData() Data {
	return Data{
		...
		TopIncidents: []Incident{
			{DeviceID: "d1", RuleID: "KUT-0001", Severity: "HIGH", Count: 4, LastSeen: time.Now()},
			{DeviceID: "d2", RuleID: "KUT-0002", Severity: "CRITICAL", Count: 9, LastSeen: time.Now()},
		},
	}
}
...
func TestRenderMarkdownTechnical(t *testing.T) {
	...
	for _, want := range []string{"# Teknik Güvenlik Raporu", "## Filo durumu", "## Tehdit etkinliği",
		"## Önem dağılımı", "| CRITICAL | 2 |", "## Öne çıkan olaylar", "KUT-0002", "kiracı: acme"} {
