...
func (m *memStore) AppendCustody(_ context.Context, evidenceID string, e evidence.CustodyEntry) error {
	...
	for _, ex := range m.custody[evidenceID] {
		if ex.Seq == e.Seq {
			return errDupSeq
		}
	}
...
var errDupSeq = errorsNewCustody("dup seq")

func errorsNewCustody(s string) error { return custodyErr(s) }

type custodyErr string

func (e custodyErr) Error() string { return string(e) }
