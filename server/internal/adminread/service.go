...
import (
	...
	"kut.corp/suite/server/internal/complianceframework"
	...
	"kut.corp/suite/server/internal/ueba"
)
...
type Store interface {
	...
	ListCustody(ctx context.Context, evidenceID string) ([]evidence.CustodyEntry, error)
...
// DeviceEvidence, cihazın TOPLANAN artefaktlarından kurcalamaya-dayanıklı delil
// kayıtları üretir (§23): her artefaktın kaydedilmiş SHA-256'sından (yeniden okumadan)
...
func (s *Service) DeviceEvidence(ctx context.Context, deviceID string) ([]EvidenceDTO, error) {
	...
	for _, r := range rows {
		// Genesis zamanı, artefaktın KALICI toplama anıdır (r.CollectedAt) — böylece
		// delil kaydı YENİDEN ÜRETİLEBİLİR: aynı artefakt her çağrıda AYNI gözetim
		// hash'ini verir (kurcalama-kanıtı için şart). time.Now kullanılsaydı hash her
		// istekte değişir ve doğrulanabilirlik anlamsızlaşırdı.
		collectedAt := r.CollectedAt
		ev := evidence.FromHash(r.ID, r.DeviceID, "kut-agent", "live-collect", r.Path, r.SHA256,
			int64(r.Size), func() time.Time { return collectedAt })
		// KALICI gözetim kayıtlarını (seq>=1) genesis'in üzerine ekle; zincir baştan doğrulanır.
		if entries, cerr := s.store.ListCustody(ctx, r.ID); cerr == nil {
			ev.Custody = append(ev.Custody, entries...)
		}
		ok, _ := ev.Verify()
		out = append(out, EvidenceDTO{Evidence: ev, Verified: ok})
	}
	return out, nil
}

// RecordCustody, bir delile (artefakt) KALICI bir gözetim olayı ekler (§23): mevcut
...
func (s *Service) RecordCustody(ctx context.Context, deviceID, artifactID, actor, action string) (evidence.CustodyEntry, error) {
	...
	var found *ArtifactRow
	for i := range rows {
		if rows[i].ID == artifactID {
			found = &rows[i]
			break
		}
	}
	if found == nil {
		return evidence.CustodyEntry{}, ErrArtifactNotFound
	}
	collectedAt := found.CollectedAt
	ev := evidence.FromHash(found.ID, found.DeviceID, "kut-agent", "live-collect", found.Path,
		found.SHA256, int64(found.Size), func() time.Time { return collectedAt })
	entries, err := s.store.ListCustody(ctx, artifactID)
	if err != nil {
		return evidence.CustodyEntry{}, err
	}
	ev.Custody = append(ev.Custody, entries...)
	// Zincirin ucuna hash-bağlı yeni kayıt. Zaman MİKROSANİYEye yuvarlanır: hash
	// At.UnixNano()'ya dayanır ve Postgres TIMESTAMPTZ mikrosaniye çözünürlüğündedir;
	// yuvarlamazsak DB'den okununca nanosaniye kaybolur ve Verify hash'i uyuşmaz.
	newEntry := ev.Append(actor, action, func() time.Time { return time.Now().UTC().Truncate(time.Microsecond) })
	if err := s.store.AppendCustody(ctx, artifactID, newEntry); err != nil {
		return evidence.CustodyEntry{}, err
	}
	return newEntry, nil
}

// ErrInvalidCustodyAction, geçersiz/izin verilmeyen bir gözetim eylemi için döner.
var ErrInvalidCustodyAction = errors.New("adminread: geçersiz gözetim eylemi")

// ErrArtifactNotFound, verilen kimlikte artefakt bulunamadığında döner.
var ErrArtifactNotFound = errors.New("adminread: artefakt bulunamadı")
