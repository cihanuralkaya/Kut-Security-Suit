...
func newVerifier(t *testing.T, key *rsa.PrivateKey, kid string, now time.Time) *Verifier {
	t.Helper()
	return &Verifier{
		Issuer:   "https://idp.kut.corp",
		Audience: "kut-console",
...
func baseClaims(now time.Time) map[string]any {
	return map[string]any{
		"iss":   "https://idp.kut.corp",
		"aud":   "kut-console",
		"sub":   "user-123",
		"email": "alice@kut.corp",
		"name":  "Alice Analyst",
...
func TestVerifyHappyPath(t *testing.T) {
	...
	if claims.Subject != "user-123" || claims.Email != "alice@kut.corp" || claims.Name != "Alice Analyst" {
		t.Fatalf("unexpected claims: %+v", claims)
	}
	if len(claims.Audience) != 1 || claims.Audience[0] != "kut-console" {
...
func TestVerifyAudienceArray(t *testing.T) {
	...
	v := newVerifier(t, key, "k1", now)
	c := baseClaims(now)
	c["aud"] = []any{"other-app", "kut-console"}
