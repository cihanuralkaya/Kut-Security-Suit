...
func TestEvaluateDefaultRules(t *testing.T) {
	...
	cases := []struct {
		...
		{"SECURITY", "watchdog kurcalama tespit edildi", "KUT-0001", "CRITICAL", "T1562"},
		{"SECURITY", "imzasız/sahte script reddedildi: cmd-9", "KUT-0002", "HIGH", "T1059"},
		{"SECURITY", "sahte/bozuk güncelleme reddedildi", "KUT-0003", "HIGH", "T1195"},
		{"SECURITY", "anomali: olağandışı süreç davranışı", "KUT-0004", "HIGH", "T1055"},
		{"POLICY_VIOLATION", "yasaklı süreç sonlandırıldı: game.exe", "KUT-0005", "HIGH", "T1204"},
		{"NETWORK_DISCOVERY", "10 komşu bulundu", "KUT-0006", "LOW", "T1046"},
	}
	for _, c := range cases {
...
// KUT-0007 varsayılan kuralı: şüpheli PROCESS regex'i.
func TestDefaultSuspiciousProcessRule(t *testing.T) {
	...
	if len(d) == 0 || d[0].RuleID != "KUT-0007" {
		t.Fatalf("KUT-0007 powershell -EncodedCommand eşleşmeliydi: %+v", d)
	}
}
