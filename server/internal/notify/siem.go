...
//
//	CEF:0|KUT|Suite|<ver>|<signatureID>|<name>|<sev>|<extension>
func formatCEF(a Alert, product, version string) string {
	...
	return fmt.Sprintf("CEF:0|KUT|%s|%s|%s|%s|%d|%s",
...
//
//	LEEF:2.0|KUT|Suite|<ver>|<eventID>|<attrs>
...
func (n *SyslogNotifier) send(a Alert) {
	...
	line := fmt.Sprintf("<%d>%s %s KUT: %s", syslogPriority(a.Severity),
