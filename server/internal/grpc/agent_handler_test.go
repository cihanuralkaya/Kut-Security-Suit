...
type fakeProvider struct {
	mu     sync.Mutex
	bundle *kutv1.PolicyBundle
}

func (f *fakeProvider) set(version string) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.bundle = &kutv1.PolicyBundle{PolicyVersion: version}
}
func (f *fakeProvider) CurrentPolicy(_ context.Context, _ string) (*kutv1.PolicyBundle, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	return f.bundle, nil
}
...
func TestStreamPolicyLoopInitialAndPush(t *testing.T) {
	...
	send := func(b *kutv1.PolicyBundle) error { sent <- b.GetPolicyVersion(); return nil }
