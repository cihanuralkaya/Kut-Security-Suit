...
#define KUT_MAX_PIDS 8
static HANDLE     g_Pids[KUT_MAX_PIDS];
 ...
 * the agent connects. Adjust to the real install path (e.g. %ProgramFiles%\KUT\). */
...
VOID KutProtectInit(VOID) { ExInitializeFastMutex(&g_Lock); g_PidCount = 0; }
...
VOID KutRegisterProtectedPid(HANDLE Pid)
{
    ...
    if (g_PidCount < KUT_MAX_PIDS) g_Pids[g_PidCount++] = Pid;
...
BOOLEAN KutIsProtectedFilePath(PCUNICODE_STRING Path)
{
...
BOOLEAN KutIsProtectedImagePath(PCUNICODE_STRING Path) { return KutIsProtectedFilePath(Path); }
...
{
    return EndsWithCI(Path, L"\\Services\\kutflt") ||
           EndsWithCI(Path, L"\\KUT");
}
