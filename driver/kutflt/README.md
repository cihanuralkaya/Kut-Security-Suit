# kutflt — KUT Tamper-Protection MiniFilter (kernel driver)
...
**görmez** (Go araç zinciri `.go` içermeyen dizinleri atlar). Derlenmiş/imzalı `kutflt.sys`
...
- **Dosya koruması** (`kutflt.c`): ajan ikilisi + config üzerinde yazma/silme/rename
  IRP'lerini (`IRP_MJ_CREATE`, `IRP_MJ_SET_INFORMATION`) `STATUS_ACCESS_DENIED` ile reddeder.
- **User↔kernel port** (`comms.c`): `\KutFltPort` üzerinden ajana durum + engellenen
  kurcalama olayları iletir (`inc/kutflt_ioctl.h` ABI'si).
...
fltmc filters                      # kutflt eklendi mi doğrula
.\install-dev.ps1 -Action uninstall
```

Sürücü `System32\drivers\kutflt.sys`'e kopyalanınca, Go ajanının
`tamperprotect.KernelDriverProbe()` fonksiyonu `(true,"kutflt")` döner ve başlangıç
SECURITY olayı `tamper_level="kernel"` gösterir (kod değişikliği gerekmez).

## Ajan arayüzü

`inc/kutflt_ioctl.h` = paylaşılan ABI. Go tarafı:
...
(`fsfcomm@microsoft.com`) kalıcı bir altitude talep edip `src/kutflt.h` + `kutflt.inf`
içinde değiştirin.

## Risk & kurtarma

- Sürücü `demand-start` + `ErrorControl=normal` → boot'u ASLA bloklamaz, opt-in.
- Bozulursa: `fltmc unload kutflt` + `sc delete kutflt`; kaldıramazsa **Safe Mode**'da
...
4. `kutflt.sys`'i attestation/WHQL imzalamasına gönder; Microsoft-imzalı katalog dağıt.
