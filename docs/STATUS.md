# Durum Raporu — KUT Security Suite
# Status Report — KUT Security Suite
...
| Sunucu-taraflı tespit motoru (YAML-benzeri kurallar) | ✅ | `server/internal/detect`, `grpc`, `adminapi` | birim + httptest |
...
- **Olay yineleme-tespiti (§6):** aynı olay (içerik-adresli EventID, §5) kısa pencere
  ...
  `kut_events_duplicate_total`) — retransmit/çift-gönderim çift-saymaz. `KUT_INGEST_DEDUP_*`.
...
- **Scope / ROE guardrail (§4):** yüksek-etkili operasyonlar (WIPE/QUARANTINE/LOCK/
  ...
  device/tenant/env/port seçiciler). `KUT_SCOPE_*` ile yapılandırılır; enforce +
  denetim (would-deny) modları; `kut_scope_denied_total` metriği. Bkz.
...
- **Tespit:** çok-sinyal **korelasyon** (yüksek-güven zincir), **yanal hareket** (netconn
  ...
  geliştirilmiş **DGA** (ünsüz-dizisi), **YAML içe aktarma**, **YARA-tarzı içerik tarama**,
...
- **Operasyonel uçlar:** veri sahibi başvuru akışı (KVKK erişim/silme) **YAPILDI**
  ...
  denetim/audit modu KUT_ENFORCE_AUDIT ile) — gerçek sandbox ileri faz.
...
# db/schema.sql yükle, KUT_* env ayarla, bin/c2 ve bin/agent çalıştır
...
| Server-side detection engine (YAML-like rules) | done | `server/internal/detect`, `grpc`, `adminapi` | unit + httptest |
...
- **Detection:** multi-signal **correlation** (high-confidence chain), **lateral movement**
  ...
  **YAML import**, **YARA-style content scanning**, Detection-as-Code lifecycle, expanded
...
go run ./tools/otasign -genkey -out ./ota-keys
go run ./tools/adminseed -email a@x -password '...' -role ADMIN
# load db/schema.sql, set KUT_* env, run bin/c2 and bin/agent
...
- `tools/mkicon` - pure-Go app-icon generator (.ico + embedded `*_windows_amd64.syso`
  PE resource) from `assets/kut-logo.png`; see `docs/BRANDING.md`.
