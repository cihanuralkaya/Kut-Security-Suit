...
KUT, uç-nokta ajan telemetrisinin yanı sıra HARİCİ kaynaklardan (güvenlik duvarı,
...
```
POST /api/ingest
Authorization: Bearer <KUT_INGEST_TOKEN>
```

- Uç, yalnız `KUT_INGEST_TOKEN` ayarlıysa AÇIKtır; aksi halde `404`. Token sabit-zaman
  karşılaştırılır. Güçlü, rastgele bir token kullanın.
- Hız sınırı: istemci IP başına token-bucket (`KUT_INGEST_RATE_PER_SEC`, varsayılan 50,
...
|---|---|---|---|
| `Content-Type: application/json` (KUT şeması) | `source`+`message` alanlı JSON dizi/nesne | `source` | `severity` alanı |
...
> `logcheck` CLI ile KUT'in nasıl ayrıştırdığını görün (ağ/sunucu gerekmez):
> ```bash
...
**winlogbeat** (Windows olayları → KUT):
...
Beyond endpoint agent telemetry, KUT ingests logs from EXTERNAL sources (firewall,
...
- Open only when `KUT_INGEST_TOKEN` is set (else `404`); constant-time token compare.
- Per-client-IP token-bucket rate limit (`KUT_INGEST_RATE_PER_SEC`, default 50, cap 2×);
...
|---|---|---|---|
| `application/json` (KUT schema) | JSON array/object with `source`+`message` | `source` | `severity` |
