# KUT V2 Roadmap — Uygulanabilirlik ve Boşluk Analizi (Gap Analysis)
...
>
> **Canlı uçlara bağlananlar (bu oturum):** §27 AI asistanı (`POST /api/ai/analyze`),
> §14 trace (HTTP ara-katmanı, W3C traceparent), §29/§30/§45 connector runner
> (`KUT_CONNECTORS_FILE` → çek/normalize/depola), §35 ABAC (`POST /api/iam/abac/evaluate`).
>
> **Kalıcılık katmanı da TAMAMLANDI (DB şeması + store + uç, hepsi CI'da "DB yolu
> (PostgreSQL)" işiyle gerçek Postgres'e karşı doğrulandı):**
> - **§23 evidence** kalıcı gözetim-zinciri: `evidence_custody` tablosu (ekle-yalnız,
>   hash-zincirli) + `GET /api/devices/{id}/evidence` + `POST .../custody`. Genesis
>   kararlı (yeniden üretilebilir); zaman mikrosaniyeye yuvarlanır (hash round-trip).
> - **§35 SCIM** kalıcı kullanıcı sağlama: `scim_users` (JSONB) + `/scim/v2/Users`
>   (POST/GET/PUT/DELETE), DB destekli iam.Provisioner.
> - **§37 MSP** kalıcı müşteri yönetimi: `msp_customers` + `/api/msp/customers`.
>
> Kalıcılık DB modunda; demo modunda bellek-içi. **§35 SAML** ve **§10 Windows-ayrıcalık
> tespiti** bilinçli belgelenmiş seam'ler olarak kalır (stdlib ötesi gerektirir).
...
|---|--------|:-----:|----------------|---------------------------|
| 4 | Central Scope / ROE / Guardrails | ✅ VAR | `server/internal/scope` (Engine.Authorize, fail-closed, excluded-wins, ROE action-gate, wildcard/CIDR selector) + admin servis kapısı (WIPE/QUARANTINE/LOCK/RESTART) + `KUT_SCOPE_*` config + metrikler | **Bu oturumda uygulandı.** Yüksek-etkili her op enqueue'dan önce `guardScope`'tan geçer; enforce/denetim modları. Geriye uyumlu (motor bağlı değilse no-op). |
...
| 13 | Secrets & Config Security | 🟡 KISMİ | `config` (env/`KUT_`), master-key base64 | Eksik: KMS/Vault/secret-manager entegrasyonu, imzalama/CA anahtarları için key-rotation. Arayüz saf-Go; sağlayıcı dış bağımlılık (opsiyonel). |
...
| 20 | Detection-as-Code | ✅ VAR | `ruleyaml` + `/api/detections/rules`+`/test` + `detectsign` (imzalı kural) + lifecycle | Karşılanıyor (YAML tabanlı, imzalı). |
...
| 44 | Detection Registry / Marketplace | ⬜ YOK | `ruleyamlimport` (kısmi adım) | Versiyonlu kural registry'si. Saf-Go. |
