...
Bu belge, KUT ajanının kurcalamaya (tamper) karşı korunmasını iki katmanda tanımlar:
...
`tamperprotect.KernelDriverProbe()` (platforma özgü), gelecekteki KUT MiniFilter
sürücüsünün (`kutflt.sys`) varlığını yoklar. Bu depo sürücü **sevk etmez** → yoklama
...
This document describes KUT agent tamper protection in two layers: (1) the **userland
...
`tamperprotect.KernelDriverProbe()` (platform-specific) probes for the future KUT
MiniFilter driver (`kutflt.sys`). This repo ships **no** driver → the probe returns
